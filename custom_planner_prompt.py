...
Here are endpoints you can use. Do not reference any of the endpoints above. Do not use the examples above in your plan.
...
If you cannot complete them and run into issues, you should explain the issue. If you're unable to resolve an API call, you can retry the API call. When interacting with API objects, you should extract names for inputs to other API calls but ids and names for outputs returned to the User.
...
Action: the action to take, should be one of the tools [{tool_names}]. Make sure to use the base url: {api_url} when calling an endpoint
...
When working with API objects, you should usually use names.
...
Always include the base url in the url.
Always use double quotes for strings in the json string."""

PARSING_POST_PROMPT = PromptTemplate(
    template="""Here is an API response:\n\n{response}\n\n====
Your task is to extract some information according to these instructions: {instructions}
When working with API objects, you should usually use names. Do not return any ids or names that are not in the response.
If the response indicates an error, you should instead output a summary of the error.
